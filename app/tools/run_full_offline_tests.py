        ...
        def connect_and_join() -> None:
            try:
                ...
                if hasattr(win, "_schedule_async"):
                    ...
                    QTimer.singleShot(
                        300, lambda: win._schedule_async(win.bridge.joinChannel, "127.0.0.1:#test")
                    )
                else:
                    ...
                    asyncio.get_event_loop().call_later(
                        0.3,
                        lambda: asyncio.ensure_future(win.bridge.joinChannel("127.0.0.1:#test")),
                    )
        ...
        def checks() -> None:
            try:
                ...
                try:
                    ...
                    self.log(
                        "[OK] YouTube iframe embed present"
                        if yt_iframe in html
                        else "[WARN] YouTube iframe embed missing"
                    )
                    # Expect <img> tag for placeholder image URL
                    img_url = "https://via.placeholder.com/200x100.png"
                    self.log(
                        "[OK] Image <img> tag present"
                        if img_url in html
                        else "[WARN] Image <img> tag missing"
                    )
                ...
                try:
                    ...
                    self.log(
                        "[OK] Video panel visible" if vis else "[WARN] Video panel not visible"
                    )
                except Exception:
                ...
                try:
                    ...
                    self.log(
                        "[OK] BrowserWindow exists" if bw else "[WARN] BrowserWindow not created"
                    )
                except Exception:
